...
export class Device extends TypedEmitter<DeviceEvents> {
  ...
  public async runShellCommandAdb(command: string): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      const adbPath = AdbUtils.getPathToAdbExecutable();
      const cmd = adbPath
      ...
      adb.stdout.on('data', (data) => {
        ...
        log.info(`stdout: ${data.toString().replace(/\n$/, '')}`);
      });
      ...
      adb.on('error', (error: Error) => {
        log.error(`Failed to spawn adb process.\n${error.stack}`);
  ...
  public async getNetInterfaces(): Promise<NetInterface[]> {
    ...
    const output = await this.runShellCommandAdbKit(
  ...
  private scheduleInfoUpdate(): void {
    ...
    if (++this.updateCount > Device.MAX_UPDATES_COUNT) {
      console.error(
        this.TAG,
        'The maximum number of attempts to fetch device info has been reached.'
      );
      return;
    }
  ...
  private fetchDeviceInfo = (): void => {
    log.info('fetchDeviceInfo(): Fetching device info');
    if (this.connected) {
  ...
  public async startServer(): Promise<number | undefined> {
    log.info('startServer(): Starting scrcpy server');
