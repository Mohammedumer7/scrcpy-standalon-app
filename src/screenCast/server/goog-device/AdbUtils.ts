...
export class AdbUtils {
    ...
    const all = list.map(async (entry) => {
      if (entry.isSymbolicLink()) {
        const stat = await this.stats(
          serial,
          path.join(pathString, entry.name)
        );
  ...
  private static async createHttpRequest(
    ...
    const client = AdbExtended.createClient();
    const socket = await client.openLocal(
      serial,
      `localabstract:${unixSocketName}`
    );
    ...
    const message: http.IncomingMessage = await new Promise(
      (resolve, reject) => {
  ...
  private static patchWebSocketDebuggerUrl(
    host: string,
    serial: string,
    socket: string,
    url: string
  ...
  public static async getRemoteDevtoolsVersion(
    ...
    const data = await this.createHttpRequest(
      serial,
      socket,
      `${proto}${fakeHost}/json/version`
    );
    if (!data) {
      throw Error('Empty response');
    }
  ...
  public static async getRemoteDevtoolsTargets(
    ...
    return list.map((target) => {
      ...
      if (devtoolsFrontendUrl) {
        ...
        const ws = this.patchWebSocketDebuggerUrl(
          host,
          serial,
          socket,
  ...
  public static async getRemoteDevtoolsInfo(
    ...
    list.forEach((socket) => {
      const v = this.getRemoteDevtoolsVersion(host, serial, socket).catch(
        (error: Error) => {
      ...
      const t = this.getRemoteDevtoolsTargets(host, serial, socket).catch(
        (error: Error) => {
  ...
  public static getPathToAdbExecutable(): string {
    let adbPath = getAdbPath();
    log.info('Screencast: Getting path to ADB executable');
    log.info(`Screencast: isBuild: ${isBuild}`)
    return adbPath;
  }
}
