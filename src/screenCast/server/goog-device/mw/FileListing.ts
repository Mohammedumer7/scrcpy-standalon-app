...
export class FileListing extends Mw {
  ...
  private static handleNewChannel(
    serial: string,
    channel: Multiplexer,
    ...
    if (data.length < 4) {
      console.error(
        `[${FileListing.TAG}]`,
        `Invalid message. Too short (${data.length})`
      );
      return;
    }
    let offset = 0;
    ...
    switch (cmd) {
      ...
      case Protocol.RECV:
        ...
        FileListing.handle(cmd, serial, pathString, channel).catch(
          (error: Error) => {
      ...
      default:
        console.error(
          `[${FileListing.TAG}]`,
          `Invalid message. Wrong command (${cmd})`
        );
