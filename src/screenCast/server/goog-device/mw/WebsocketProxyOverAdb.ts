...
export class WebsocketProxyOverAdb extends WebsocketProxy {
  public static processRequest(
    ...
    if (typeof remote !== 'string' || !remote) {
      ws.close(
        4003,
        `[${this.TAG}] Invalid value "${remote}" for "remote" parameter`
      );
      return;
    }
    if (typeof udid !== 'string' || !udid) {
      ws.close(
        4003,
        `[${this.TAG}] Invalid value "${udid}" for "udid" parameter`
      );
      return;
    }
    if (path && typeof path !== 'string') {
      ws.close(
        4003,
        `[${this.TAG}] Invalid value "${path}" for "path" parameter`
      );
      return;
    }
  ...
  public static createProxyOverAdb(
    ...
    console.log(`Attempting ADB forwarding: udid=${udid}, remote=${remote}`);
    AdbUtils.forward(udid, remote)
      .then((port) => {
        console.log(`ADB forwarding successful. Port: ${port}`);
        return service.init(`ws://127.0.0.1:${port}${path ? path : ''}`);
      })
      .catch((e) => {
        console.log('logging udid', udid);
        const msg = `ADB FORWARDING[${this.TAG}] Failed to start service: ${e.message}`;
        console.error('ADB FORWARDING', e);
        console.error(e.stack); // Log the stack trace for more details
        ws.close(4005, msg + `udid${udid}`);
      });
