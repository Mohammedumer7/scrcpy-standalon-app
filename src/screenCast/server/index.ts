...
export async function StartCastingServer() {
  ...
  //   const { ControlCenter } = await import(
  //     './appl-device/services/ControlCenter'
  //   );
  //   const { DeviceTracker } = await import('./appl-device/mw/DeviceTracker');
  //   const { WebDriverAgentProxy } = await import(
  //     './appl-device/mw/WebDriverAgentProxy'
  //   );
  ...
  /// #endif
  await Promise.all(loadPlatformModulesPromises)
    .then(() => {
    ...
    .catch((error) => {
      console.error('Error in loading SERVER', error);
      exit('1');
    });
  function exit(signal: string) {
    console.log(`\nReceived signal ${signal}`, interrupted);
    if (interrupted) {
